...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product m-5">
      <div>
    <img class="product-image" src=${image}></img>
      </div>
      <h3 class ="highlight-colour-one">${product.title}</h3>
      <p>Category: ${product.category}</p>
      <h4>Rating: <span class="highlight-colour-two">${product.rating.rate}</span> Rated: <span class="highlight-colour-two">${product.rating.count}</span>  </h4>
      
      <h2 class ="highlight-colour-one">Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="add-to-cart-button ">add to cart</button>
      <button id="details-btn" class="detail-now">Details</button></div>
      `;
